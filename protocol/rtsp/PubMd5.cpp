 ...
 * multiple of 4.
 */
static void pub_md5_encode(unsigned char *output, unsigned int * input, unsigned int len)
{
	unsigned int    i, j;
 ...
 * multiple of 4.
 */
static void pub_md5_decode(unsigned int * output, unsigned char const *input, unsigned int len)
{
	unsigned int    i, j;
 ...
 */
static void pub_md5_transform(unsigned int state[4], const unsigned char block[64])
{
  unsigned int a = state[0], b = state[1], c = state[2], d = state[3], x[16];

	pub_md5_decode(x, block, 64);
...
void pub_md5_update(PUB_MD5_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	...
	if (inputLen >= partLen) {
		...
		pub_md5_transform(context->state, context->buffer);

		for (i = partLen; i + 63 < inputLen; i += 64) {
			pub_md5_transform(context->state, &input[i]);
		}
		index = 0;
	} else {
		i = 0;
...
void pub_md5_final(unsigned char digest[16], PUB_MD5_CTX * context)
{
	unsigned char   bits[8];
	unsigned int    index, padLen;

	/* Save number of bits */
	pub_md5_encode(bits, context->count, 8);
	...
	pub_md5_update(context, PADDING, padLen);

	/* Append length (before padding) */
	pub_md5_update(context, bits, 8);
	/* Store state in digest */
	pub_md5_encode(digest, context->state, 16);
...
{
	PUB_MD5_CTX context;
	pub_md5_init(&context);
	pub_md5_update(&context,input,inputLen);
	pub_md5_end(&context,out);
}
