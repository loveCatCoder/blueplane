...
int sdp_message_y_value_set(sdp_message_t * sdp, char *name)
{
	if (sdp == NULL)
		return -1;
	sdp->y_value = name;
...
char *sdp_message_y_value_get(sdp_message_t * sdp)
{
	if (sdp == NULL)
		return NULL;
	return sdp->y_value;
...
int sdp_message_i_info_set(sdp_message_t * sdp, int pos_media, char *info)
{
	...
	med = (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	if (med == NULL)
...
int sdp_message_e_email_add(sdp_message_t * sdp, char *email)
{
	if (sdp == NULL)
		return sip_BADPARAMETER;
	sip_list_add(&sdp->e_emails, email, -1);
	return sip_SUCCESS;
}
...
char *sdp_message_e_email_get(sdp_message_t * sdp, int pos)
{
	if (sdp == NULL)
		return NULL;
	if (sip_list_size(&sdp->e_emails) > pos)
		return (char *) (sdp_media_t *)sip_list_get(&sdp->e_emails, pos);
	return NULL;
}
...
int sdp_message_p_phone_add(sdp_message_t * sdp, char *phone)
{
	if (sdp == NULL)
		return sip_BADPARAMETER;
	sip_list_add(&sdp->p_phones, phone, -1);
	return sip_SUCCESS;
}
...
char *sdp_message_p_phone_get(sdp_message_t * sdp, int pos)
{
	if (sdp == NULL)
		return NULL;
	if (sip_list_size(&sdp->p_phones) > pos)
		return (char *) (sdp_media_t *)sip_list_get(&sdp->p_phones, pos);
	return NULL;
}
							 ...
							 char *addr_multicast_int)
{
	...
	if (sdp == NULL)
		return sip_BADPARAMETER;
	if ((pos_media != -1) && (sip_list_size(&sdp->m_medias) < pos_media + 1))
		return -1;
	...
	med = (sdp_media_t *) (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	sip_list_add(&med->c_connections, conn, -1);
	return sip_SUCCESS;
}

/* this method should be internal only... */
sdp_connection_t *sdp_message_connection_get(sdp_message_t * sdp, int pos_media,
											 int pos)
{
	sdp_media_t *med;
	...
	if (med == NULL)
		return NULL;
	return (sdp_connection_t *) (sdp_media_t *)sip_list_get(&med->c_connections, pos);
}
							...
							char *bandwidth)
{
	...
	if (pos_media == -1) {
		sip_list_add(&sdp->b_bandwidths, band, -1);
		return sip_SUCCESS;
	}
	med = (sdp_media_t *) (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	sip_list_add(&med->b_bandwidths, band, -1);
	return sip_SUCCESS;
}

sdp_bandwidth_t *sdp_message_bandwidth_get(sdp_message_t * sdp, int pos_media,
										   int pos)
{
	sdp_media_t *med;
	...
	if (sdp == NULL)
		return NULL;
	if (pos_media == -1)
		return (sdp_bandwidth_t *) (sdp_media_t *)sip_list_get(&sdp->b_bandwidths, pos);
	med = (sdp_media_t *) (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	if (med == NULL)
		return NULL;
	return (sdp_bandwidth_t *) (sdp_media_t *)sip_list_get(&med->b_bandwidths, pos);
}
...
int sdp_message_t_time_descr_add(sdp_message_t * sdp, char *start, char *stop)
{
	...
	td->t_stop_time = stop;
	sip_list_add(&sdp->t_descrs, td, -1);
	return sip_SUCCESS;
}
...
char *sdp_message_t_start_time_get(sdp_message_t * sdp, int pos_td)
{
	...
	if (sdp == NULL)
		return NULL;
	td = (sdp_time_descr_t *) (sdp_media_t *)sip_list_get(&sdp->t_descrs, pos_td);
	if (td == NULL)
		return NULL;
...
int sdp_message_r_repeat_add(sdp_message_t * sdp, int pos_time_descr, char *field)
{
	...
	if (sdp == NULL)
		return sip_BADPARAMETER;
	td = (sdp_time_descr_t *) (sdp_media_t *)sip_list_get(&sdp->t_descrs, pos_time_descr);
	if (td == NULL)
		return sip_UNDEFINED_ERROR;
	sip_list_add(&td->r_repeats, field, -1);
	return sip_SUCCESS;
}
							   ...
							   int pos_repeat)
{
	...
	if (td == NULL)
		return NULL;
	return (char *) (sdp_media_t *)sip_list_get(&td->r_repeats, pos_repeat);
}
							...
							char *att_field, char *att_value)
{
	...
	if (pos_media == -1) {
		sip_list_add(&sdp->a_attributes, attr, -1);
		return sip_SUCCESS;
	}
	med = (sdp_media_t *) (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	sip_list_add(&med->a_attributes, attr, -1);
	return sip_SUCCESS;
}
...
sdp_message_a_attribute_del(sdp_message_t * sdp, int pos_media, char *att_field)
{
	...
	if ((pos_media != -1) && (sip_list_size(&sdp->m_medias) < pos_media + 1))
		return sip_UNDEFINED_ERROR;
	if (pos_media == -1) {
		for (i = 0; i < sip_list_size(&sdp->a_attributes);) {
			attr = (sdp_attribute_t *)sip_list_get(&sdp->a_attributes, i);
			if (strcmp(attr->a_att_field, att_field) == 0) {
				sip_list_remove(&sdp->a_attributes, i);
	...
	if (med == NULL)
		return sip_UNDEFINED_ERROR;
	for (i = 0; i < sip_list_size(&med->a_attributes);) {
		attr = (sdp_attribute_t *)sip_list_get(&med->a_attributes, i);
		if (strcmp(attr->a_att_field, att_field) == 0) {
			sip_list_remove(&med->a_attributes, i);
			sdp_attribute_free(attr);
		} else
			i++;
	}
									 ...
									 char *att_field, int pos_attr)
{
	...
	if (pos_media == -1) {
		...
		} else if ((attr = (sdp_attribute_t *)sip_list_get(&sdp->a_attributes, pos_attr)) != NULL) {
			sip_list_remove(&sdp->a_attributes, pos_attr);
			sdp_attribute_free(attr);
		}
		return sip_SUCCESS;
	}
	...
	for (i = 0; i < sip_list_size(&med->a_attributes);) {
		if (pos_attr == -1) {
			...
			if (strcmp(attr->a_att_field, att_field) == 0) {
				...
				i++;
		} else if ((attr = (sdp_attribute_t *)sip_list_get(&med->a_attributes, pos_attr)) != NULL) {
			sip_list_remove(&med->a_attributes, pos_attr);
			sdp_attribute_free(attr);
		}
	}
	return sip_SUCCESS;
...
sdp_attribute_t *sdp_message_attribute_get(sdp_message_t * sdp, int  pos_media,
										   int pos)
{
	sdp_media_t *med;
	...
	if (sdp == NULL)
		return NULL;
	if (pos_media == -1)
		return (sdp_attribute_t *) (sdp_media_t *)sip_list_get(&sdp->a_attributes, pos);
	med = (sdp_media_t *) (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	if (med == NULL)
		return NULL;
	return (sdp_attribute_t *) (sdp_media_t *)sip_list_get(&med->a_attributes, pos);
}
...
int sdp_message_endof_media(sdp_message_t * sdp, int i)
{
	...
	if (i == -1)
		return sip_SUCCESS;
	if (!sip_list_eol(&sdp->m_medias, i))
		return sip_SUCCESS;	/* not end of list */
	return sip_UNDEFINED_ERROR;	/* end of list */
}

int
sdp_message_m_media_add(sdp_message_t * sdp, char *media,
						char *port, char *proto, char *fmt_list)
{
	int i;
	sdp_media_t *med;
	...
	med->m_proto = proto;
	//med->m_fmt_list = fmt_list;	
	sip_list_add(&med->m_fmt_list, fmt_list, -1);
	sip_list_add(&sdp->m_medias, med, -1);
	return sip_SUCCESS;
}

char *sdp_message_m_media_get(sdp_message_t * sdp, int pos_media)
{
	sdp_media_t *med = (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
...
char *sdp_message_m_fmt_list_get(sdp_message_t * sdp, int pos_media,  int pos)
{
#if 0
	sdp_media_t *med = (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);
	...
	if (med == NULL)
		return NULL;
	return med->m_fmt_list;
#endif
	sdp_media_t *med = (sdp_media_t *)sip_list_get(&sdp->m_medias, pos_media);

	if (med == NULL)
		return NULL;
	return (char *) (sdp_media_t *)sip_list_get(&med->m_fmt_list, pos);
...
int sdp_message_m_payload_add(sdp_message_t * sdp, int pos_media, char *payload)
{
	...
	if (med == NULL)
		return sip_BADPARAMETER;
	sip_list_add(&med->m_payloads, payload, -1);
	return sip_SUCCESS;
}
...
char *sdp_message_m_payload_get(sdp_message_t * sdp, int pos_media, int pos)
{
	...
	if (med == NULL)
		return NULL;
	return (char *) (sdp_media_t *)sip_list_get(&med->m_payloads, pos);
}
...
int sdp_message_m_payload_del(sdp_message_t * sdp, int pos_media, int pos)
{
	...
	if (med == NULL)
		return sip_BADPARAMETER;
	if ((payload = (char *)sip_list_get(&med->m_payloads, pos)) == NULL)
		return sip_UNDEFINED_ERROR;
	sip_list_remove(&med->m_payloads, pos);
	sip_free(payload);
	return sip_SUCCESS;
}
