...
{
	*b = (sdp_bandwidth_t *) sip_malloc(sizeof(sdp_bandwidth_t));
	if (*b == NULL)
		return sip_NOMEM;
	(*b)->b_bwtype = NULL;
	(*b)->b_bandwidth = NULL;
...
void sdp_bandwidth_free(sdp_bandwidth_t * b)
{
	if (b == NULL)
		return;
	sip_free(b->b_bwtype);
	sip_free(b->b_bandwidth);
	sip_free(b);
}

int sdp_time_descr_init(sdp_time_descr_t ** td)
{
	*td = (sdp_time_descr_t *) sip_malloc(sizeof(sdp_time_descr_t));
	if (*td == NULL)
		return sip_NOMEM;
	(*td)->t_start_time = NULL;
	(*td)->t_stop_time = NULL;
	sip_list_init(&(*td)->r_repeats);
	return sip_SUCCESS;
}
...
void sdp_time_descr_free(sdp_time_descr_t * td)
{
	if (td == NULL)
		return;
	sip_free(td->t_start_time);
	sip_free(td->t_stop_time);
	sip_list_ofchar_free(&td->r_repeats);
	sip_free(td);
}

int sdp_key_init(sdp_key_t ** key)
{
	*key = (sdp_key_t *) sip_malloc(sizeof(sdp_key_t));
	if (*key == NULL)
		return sip_NOMEM;
	(*key)->k_keytype = NULL;
	(*key)->k_keydata = NULL;
...
void sdp_key_free(sdp_key_t * key)
{
	if (key == NULL)
		return;
	sip_free(key->k_keytype);
	sip_free(key->k_keydata);
	sip_free(key);
}

int sdp_attribute_init(sdp_attribute_t ** attribute)
{
	*attribute = (sdp_attribute_t *) sip_malloc(sizeof(sdp_attribute_t));
	if (*attribute == NULL)
		return sip_NOMEM;
...
void sdp_attribute_free(sdp_attribute_t * attribute)
{
	if (attribute == NULL)
		return;
	sip_free(attribute->a_att_field);
	sip_free(attribute->a_att_value);
	sip_free(attribute);
}

int sdp_connection_init(sdp_connection_t ** connection)
{
	*connection = (sdp_connection_t *) sip_malloc(sizeof(sdp_connection_t));
	if (*connection == NULL)
		return sip_NOMEM;
...
void sdp_connection_free(sdp_connection_t * connection)
{
	if (connection == NULL)
		return;
	sip_free(connection->c_nettype);
	sip_free(connection->c_addrtype);
	sip_free(connection->c_addr);
	sip_free(connection->c_addr_multicast_ttl);
	sip_free(connection->c_addr_multicast_int);
	sip_free(connection);
}
...
{
	int i;
	*media = (sdp_media_t *) sip_malloc(sizeof(sdp_media_t));
	if (*media == NULL)
		return sip_NOMEM;
	(*media)->m_media = NULL;
	(*media)->m_port = NULL;
	...
	(*media)->m_proto = NULL;
	//(*media)->m_fmt_list = NULL;
	i = sip_list_init(&(*media)->m_fmt_list);
	if (i != 0) {
		sip_free(*media);
		*media = NULL;
		return sip_NOMEM;
	}
	i = sip_list_init(&(*media)->m_payloads);
	if (i != 0) {
		sip_free(*media);
		*media = NULL;
		return sip_NOMEM;
	...
	(*media)->i_info = NULL;
	i = sip_list_init(&(*media)->c_connections);
	if (i != 0) {
		sip_list_ofchar_free(&(*media)->m_payloads);
		sip_free(*media);
		*media = NULL;
		return sip_NOMEM;
	}
	i = sip_list_init(&(*media)->b_bandwidths);
	if (i != 0) {
		sip_list_ofchar_free(&(*media)->m_payloads);
		sip_list_special_free(&(*media)->c_connections,
							   (void (*)(void *)) &sdp_connection_free);
		sip_free(*media);
		*media = NULL;
		return sip_NOMEM;
	}
	i = sip_list_init(&(*media)->a_attributes);
	if (i != 0) {
		sip_list_ofchar_free(&(*media)->m_payloads);
		sip_list_special_free(&(*media)->c_connections,
							   (void (*)(void *)) &sdp_connection_free);
		sip_list_special_free(&(*media)->b_bandwidths,
...
void sdp_media_free(sdp_media_t * media)
{
	if (media == NULL)
		return;
	sip_free(media->m_media);
	sip_free(media->m_port);
	//sip_free(media->m_number_of_port);
	sip_free(media->m_proto);
	//sip_free(media->m_fmt_list);
	sip_list_ofchar_free(&media->m_fmt_list);
	sip_list_ofchar_free(&media->m_payloads);
	sip_free(media->i_info);
	sip_list_special_free(&media->c_connections,
						   (void (*)(void *)) &sdp_connection_free);
	sip_list_special_free(&media->b_bandwidths,
						   (void (*)(void *)) &sdp_bandwidth_free);
	sip_list_special_free(&media->a_attributes,
...
{
	int i;
	(*sdp) = (sdp_message_t *) sip_malloc(sizeof(sdp_message_t));
	if (*sdp == NULL)
		return sip_NOMEM;
	...
	i = sip_list_init(&(*sdp)->e_emails);
	if (i != 0) {
		sip_list_ofchar_free(&(*sdp)->e_emails);
		sip_free(*sdp);
		*sdp = NULL;
		return sip_NOMEM;
	}

	i = sip_list_init(&(*sdp)->p_phones);
	if (i != 0) {
		sip_list_ofchar_free(&(*sdp)->e_emails);
		sip_free(*sdp);
		*sdp = NULL;
	...
	i = sip_list_init(&(*sdp)->b_bandwidths);
	if (i != 0) {
		sip_list_ofchar_free(&(*sdp)->e_emails);
		sip_list_ofchar_free(&(*sdp)->p_phones);
		sip_free(*sdp);
		*sdp = NULL;
		return sip_NOMEM;
	}

	i = sip_list_init(&(*sdp)->t_descrs);
	if (i != 0) {
		sip_list_ofchar_free(&(*sdp)->e_emails);
		sip_list_ofchar_free(&(*sdp)->p_phones);
		sip_list_special_free(&(*sdp)->b_bandwidths,
							   (void (*)(void *)) &sdp_bandwidth_free);
		sip_free(*sdp);
		*sdp = NULL;
		return sip_NOMEM;
	...
	i = sip_list_init(&(*sdp)->a_attributes);
	if (i != 0) {
		sip_list_ofchar_free(&(*sdp)->e_emails);
		sip_list_ofchar_free(&(*sdp)->p_phones);
		sip_list_special_free(&(*sdp)->b_bandwidths,
							   (void (*)(void *)) &sdp_bandwidth_free);
		sip_list_special_free(&(*sdp)->t_descrs,
	...
	if (i != 0) {
		...
		sip_list_special_free(&(*sdp)->t_descrs,
							   (void (*)(void *)) &sdp_time_descr_free);
		sip_list_special_free(&(*sdp)->a_attributes,
							   (void (*)(void *)) &sdp_attribute_free);
		sip_free(*sdp);
		*sdp = NULL;
		return sip_NOMEM;
...
static int sdp_message_parse_v(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	sdp->v_version = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (sdp->v_version == NULL)
		return sip_NOMEM;
	sip_strncpy(sdp->v_version, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_o(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	if (tmp[0] == ' ') {
		sdp->o_username = sip_strdup("firefly");
		tmp++;
	} else {
		i = __sip_set_next_token(&(sdp->o_username), tmp, ' ', &tmp_next);
		if (i != 0)
			return -1;
		tmp = tmp_next;
	}
	...
	i = __sip_set_next_token(&(sdp->o_sess_id), tmp, ' ', &tmp_next);
	if (i != 0)
		return -1;
	tmp = tmp_next;

	/* sess_id contains only numeric characters */
	i = __sip_set_next_token(&(sdp->o_sess_version), tmp, ' ', &tmp_next);
	if (i != 0)
		return -1;
	tmp = tmp_next;

	/* nettype is "IN" but will surely be extented!!! assume it's some alpha-char */
	i = __sip_set_next_token(&(sdp->o_nettype), tmp, ' ', &tmp_next);
	if (i != 0)
		return -1;
	tmp = tmp_next;

	/* addrtype  is "IP4" or "IP6" but will surely be extented!!! */
	i = __sip_set_next_token(&(sdp->o_addrtype), tmp, ' ', &tmp_next);
	if (i != 0)
		return -1;
	tmp = tmp_next;

	/* addr  is "IP4" or "IP6" but will surely be extented!!! */
	i = __sip_set_next_token(&(sdp->o_addr), tmp, '\r', &tmp_next);
	if (i != 0) {				/* could it be "\n" only??? rfc says to accept CR or LF instead of CRLF */
		i = __sip_set_next_token(&(sdp->o_addr), tmp, '\n', &tmp_next);
		if (i != 0)
			return -1;
	}
...
static int sdp_message_parse_s(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	sdp->s_name = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (sdp->s_name == NULL)
		return sip_NOMEM;
	sip_strncpy(sdp->s_name, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_f(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	/* text is interpreted as ISO-10646 UTF8! */
	/* using ISO 8859-1 requires "a=charset:ISO-8859-1 */
	sdp->f_value = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (sdp->f_value == NULL)
		return sip_NOMEM;
	sip_strncpy(sdp->f_value, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_y(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	/* text is interpreted as ISO-10646 UTF8! */
	/* using ISO 8859-1 requires "a=charset:ISO-8859-1 */
	sdp->y_value = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (sdp->y_value == NULL)
		return sip_NOMEM;
	sip_strncpy(sdp->y_value, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_i(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	/* text is interpreted as ISO-10646 UTF8! */
	/* using ISO 8859-1 requires "a=charset:ISO-8859-1 */
	i_info = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (i_info == NULL)
		return sip_NOMEM;
	sip_strncpy(i_info, equal + 1, crlf - (equal + 1));

	/* add the bandwidth at the correct place:
	   if there is no media line yet, then the "b=" is the
	   global one.
	 */
	i = sip_list_size(&sdp->m_medias);
	if (i == 0)
		sdp->i_info = i_info;
	else {
		sdp_media_t *last_sdp_media =
			(sdp_media_t *) sip_list_get(&sdp->m_medias, i - 1);
...
static int sdp_message_parse_u(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	sdp->u_uri = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (sdp->u_uri == NULL)
		return sip_NOMEM;
	sip_strncpy(sdp->u_uri, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_e(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	e_email = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (e_email == NULL)
		return sip_NOMEM;
	sip_strncpy(e_email, equal + 1, crlf - (equal + 1));

	sip_list_add(&sdp->e_emails, e_email, -1);
...
static int sdp_message_parse_p(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	/* e=email */
	/* we assume this is an EMAIL-ADDRESS */
	p_phone = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (p_phone == NULL)
		return sip_NOMEM;
	sip_strncpy(p_phone, equal + 1, crlf - (equal + 1));

	sip_list_add(&sdp->p_phones, p_phone, -1);
...
static int sdp_message_parse_c(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	i = __sip_set_next_token(&(c_header->c_nettype), tmp, ' ', &tmp_next);
	if (i != 0) {
	...
	i = __sip_set_next_token(&(c_header->c_addrtype), tmp, ' ', &tmp_next);
	if (i != 0) {
		sdp_connection_free(c_header);
		return -1;
	}
	...
	/* is MULTICAST? */
	{
		...
		if (slash != NULL && slash < crlf) {	/* it's a multicast address! */
			i = __sip_set_next_token(&(c_header->c_addr), tmp, '/', &tmp_next);
			if (i != 0) {
				sdp_connection_free(c_header);
				return -1;
			}
			...
			if (slash != NULL && slash < crlf) {	/* optionnal integer is there! */
				i = __sip_set_next_token(&(c_header->c_addr_multicast_ttl), tmp,
										  '/', &tmp_next);
				if (i != 0) {
					sdp_connection_free(c_header);
					return -1;
				...
				tmp = tmp_next;
				i = __sip_set_next_token(&(c_header->c_addr_multicast_int), tmp,
										  '\r', &tmp_next);
				if (i != 0) {
					i = __sip_set_next_token(&(c_header->c_addr_multicast_int),
											  tmp, '\n', &tmp_next);
					if (i != 0) {
						sdp_connection_free(c_header);
						return -1;
				...
				i = __sip_set_next_token(&(c_header->c_addr_multicast_ttl), tmp,
										  '\r', &tmp_next);
				if (i != 0) {
					i = __sip_set_next_token(&(c_header->c_addr_multicast_ttl),
											  tmp, '\n', &tmp_next);
					if (i != 0) {
						sdp_connection_free(c_header);
						return -1;
			...
			i = __sip_set_next_token(&(c_header->c_addr), tmp, '\r', &tmp_next);
			if (i != 0) {
				i = __sip_set_next_token(&(c_header->c_addr), tmp, '\n',
										  &tmp_next);
				if (i != 0) {
					sdp_connection_free(c_header);
					return -1;
	...
	if (i == 0)
		...
		sdp_media_t *last_sdp_media =
			(sdp_media_t *) sip_list_get(&sdp->m_medias, i - 1);
		sip_list_add(&last_sdp_media->c_connections, c_header, -1);
	}
	if (crlf[1] == '\n')
		*next = crlf + 2;
	else
...
static int sdp_message_parse_b(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	i = __sip_set_next_token(&(b_header->b_bwtype), tmp, ':', &tmp_next);
	if (i != 0) {
	...
	i = __sip_set_next_token(&(b_header->b_bandwidth), tmp, '\r', &tmp_next);
	if (i != 0) {
		i = __sip_set_next_token(&(b_header->b_bandwidth), tmp, '\n', &tmp_next);
		if (i != 0) {
			sdp_bandwidth_free(b_header);
			return -1;
		}
	...
	i = sip_list_size(&sdp->m_medias);
	if (i == 0)
		sip_list_add(&sdp->b_bandwidths, b_header, -1);
	else {
		sdp_media_t *last_sdp_media =
			(sdp_media_t *) sip_list_get(&sdp->m_medias, i - 1);
		sip_list_add(&last_sdp_media->b_bandwidths, b_header, -1);
	}
...
static int sdp_message_parse_t(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	i = __sip_set_next_token(&(t_header->t_start_time), tmp, ' ', &tmp_next);
	if (i != 0) {
	...
	i = __sip_set_next_token(&(t_header->t_stop_time), tmp, '\r', &tmp_next);
	if (i != 0) {
		i = __sip_set_next_token(&(t_header->t_stop_time), tmp, '\n', &tmp_next);
		if (i != 0) {
			sdp_time_descr_free(t_header);
			return -1;
		}
	...
	sip_list_add(&sdp->t_descrs, t_header, -1);
...
static int sdp_message_parse_r(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	index = sip_list_size(&sdp->t_descrs);
	if (index == 0)
	...
	r_header = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (r_header == NULL)
		return sip_NOMEM;
	sip_strncpy(r_header, equal + 1, crlf - (equal + 1));

	/* r field carry information for the last "t" field */
	t_descr = (sdp_time_descr_t *) sip_list_get(&sdp->t_descrs, index - 1);
	sip_list_add(&t_descr->r_repeats, r_header, -1);
...
static int sdp_message_parse_z(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	z_header = (char *)sip_malloc(crlf - (equal + 1) + 1);
	if (z_header == NULL)
		return sip_NOMEM;
	sip_strncpy(z_header, equal + 1, crlf - (equal + 1));
...
static int sdp_message_parse_k(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	if ((colon != NULL) && (colon < crlf)) {
		...
		i = __sip_set_next_token(&(k_header->k_keytype), tmp, ':', &tmp_next);
		if (i != 0) {
		...
		i = __sip_set_next_token(&(k_header->k_keydata), tmp, '\r', &tmp_next);
		if (i != 0) {
			i = __sip_set_next_token(&(k_header->k_keydata), tmp, '\n',
									  &tmp_next);
			if (i != 0) {
				sdp_key_free(k_header);
				return -1;
		...
		i = __sip_set_next_token(&(k_header->k_keytype), tmp, '\r', &tmp_next);
		if (i != 0) {
			i = __sip_set_next_token(&(k_header->k_keytype), tmp, '\n',
									  &tmp_next);
			if (i != 0) {
				sdp_key_free(k_header);
				return -1;
...
static int sdp_message_parse_a(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	if ((colon != NULL) && (colon < crlf)) {
		/* att-field is alpha-numeric */
		i = __sip_set_next_token(&(a_attribute->a_att_field), tmp, ':',
								  &tmp_next);
		if (i != 0) {
		...
		i = __sip_set_next_token(&(a_attribute->a_att_value), tmp, '\r',
								  &tmp_next);
		if (i != 0) {
			i = __sip_set_next_token(&(a_attribute->a_att_value), tmp, '\n',
									  &tmp_next);
			if (i != 0) {
				sdp_attribute_free(a_attribute);
				return -1;
		...
		i = __sip_set_next_token(&(a_attribute->a_att_field), tmp, '\r',
								  &tmp_next);
		if (i != 0) {
			i = __sip_set_next_token(&(a_attribute->a_att_field), tmp, '\n',
									  &tmp_next);
	...
	i = sip_list_size(&sdp->m_medias);
	if (i == 0)
		sip_list_add(&sdp->a_attributes, a_attribute, -1);
	else {
		sdp_media_t *last_sdp_media =
			(sdp_media_t *) sip_list_get(&sdp->m_medias, i - 1);
		sip_list_add(&last_sdp_media->a_attributes, a_attribute, -1);
	}
...
static int sdp_message_parse_m(sdp_message_t * sdp, char *buf, char **next)
{
	...
	*next = buf;
	...
	i = __sip_set_next_token(&(m_header->m_media), tmp, ' ', &tmp_next);
	if (i != 0)
	{
	...
	{	/* a number of port is specified! */
		i = __sip_set_next_token(&(m_header->m_port), tmp, '/', &tmp_next);
		if (i != 0)
		{
			sdp_media_free(m_header);
			return -1;
		...
		//i = __sip_set_next_token(&(m_header->m_number_of_port), tmp, ' ',
		//						  &tmp_next);
		//i = __sip_set_next_token(&(m_header->m_fmt_list), tmp, ' ',
		//	&tmp_next);
		//if (i != 0)
		//{
		//	sdp_media_free(m_header);
	...
	i = __sip_set_next_token(&(m_header->m_proto), tmp, ' ', &tmp_next);
	if (i != 0)
	{
		/* a few stack don't add SPACE after m_proto when rejecting all payloads */
		i = __sip_set_next_token(&(m_header->m_proto), tmp, '\r', &tmp_next);
		if (i != 0)
		{
			i = __sip_set_next_token(&(m_header->m_proto), tmp, '\n', &tmp_next);
			if (i != 0)
			{
				sdp_media_free(m_header);
				return -1;
...
/*********************************************************************************/
#if 0
	i = __sip_set_next_token(&(m_header->m_fmt_list), tmp, ' ', &tmp_next);
	if (i != 0)
	{
		/* a few stack don't add SPACE after m_proto when rejecting all payloads */
		i = __sip_set_next_token(&(m_header->m_fmt_list), tmp, '\r', &tmp_next);
		if (i != 0)
		{
			i = __sip_set_next_token(&(m_header->m_fmt_list), tmp, '\n', &tmp_next);
			if (i != 0)
...

	{
		...
		{
			i = __sip_set_next_token(&str, tmp, ' ', &tmp_next);
			if (i != 0)
			{
				sdp_media_free(m_header);
				return -1;
			...
			tmp = tmp_next;
			sip_list_add(&m_header->m_fmt_list, str, -1);
		...
		{	/* tmp_next is still less than clrf: no space */
			i = __sip_set_next_token(&str, tmp, '\r', &tmp_next);
			if (i != 0)
			{
				i = __sip_set_next_token(&str, tmp, '\n', &tmp_next);
				if (i != 0)
				{
					sdp_media_free(m_header);
					return -1;
...
/*********************************************************************************/

	{
		...
		while (more_space_before_crlf == 0)
		{
			...
			tmp = tmp_next;
			sip_list_add(&m_header->m_payloads, str, -1);
	...
	sip_list_add(&sdp->m_medias, m_header, -1);
...
int sdp_message_parse(sdp_message_t * sdp, const char *buf)
{
	...
	if (i == -1)				/* header is bad */
		...
		/* return -1; */
		sip_TRACE(sip_trace
				   (__FILE__, __LINE__, sip_INFO4, NULL,
	...
	if (sip_list_size(&sdp->e_emails) == 0 && sip_list_size(&sdp->p_phones) == 0) {
		sip_TRACE(sip_trace
				   (__FILE__, __LINE__, sip_INFO4, NULL,
					  ...
					  sdp_connection_t * conn, char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "c=");
	tmp = __sip_sdp_append_string(string, size, tmp, conn->c_nettype);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, conn->c_addrtype);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, conn->c_addr);
	if (conn->c_addr_multicast_ttl != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "/");
		tmp =
			__sip_sdp_append_string(string, size, tmp,
	...
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	*next_tmp = tmp;
	return sip_SUCCESS;
}
					 ...
					 sdp_bandwidth_t * bandwidth, char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "b=");
	tmp = __sip_sdp_append_string(string, size, tmp, bandwidth->b_bwtype);
	tmp = __sip_sdp_append_string(string, size, tmp, ":");
	tmp = __sip_sdp_append_string(string, size, tmp, bandwidth->b_bandwidth);
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
					  ...
					  sdp_time_descr_t * time_descr, char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "t=");
	tmp = __sip_sdp_append_string(string, size, tmp, time_descr->t_start_time);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, time_descr->t_stop_time);

	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);

	pos = 0;
	while (!sip_list_eol(&time_descr->r_repeats, pos)) {
		char *str = (char *) sip_list_get(&time_descr->r_repeats, pos);

		tmp = __sip_sdp_append_string(string, size, tmp, "r=");
		tmp = __sip_sdp_append_string(string, size, tmp, str);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
		pos++;
	}
...
sdp_append_key(char *string, int size, char *tmp, sdp_key_t * key, char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "k=");
	tmp = __sip_sdp_append_string(string, size, tmp, key->k_keytype);
	if (key->k_keydata != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, ":");
		tmp = __sip_sdp_append_string(string, size, tmp, key->k_keydata);
	}
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	*next_tmp = tmp;
	return sip_SUCCESS;
					 ...
					 sdp_attribute_t * attribute, char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "a=");
	tmp = __sip_sdp_append_string(string, size, tmp, attribute->a_att_field);
	if (attribute->a_att_value != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, ":");
		tmp = __sip_sdp_append_string(string, size, tmp, attribute->a_att_value);
	}
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
				 ...
				 char **next_tmp)
{
	...
	tmp = __sip_sdp_append_string(string, size, tmp, "m=");
	tmp = __sip_sdp_append_string(string, size, tmp, media->m_media);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, media->m_port);
	//if (media->m_number_of_port != NULL) {
	//if (media->m_fmt_list != NULL) {
	//	tmp = __sip_sdp_append_string(string, size, tmp, "/");
		////tmp = __sip_sdp_append_string(string, size, tmp, media->m_number_of_port);
	//	tmp = __sip_sdp_append_string(string, size, tmp, media->m_fmt_list);
	//}
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, media->m_proto);
	pos = 0;
/***********************************************************************/
	while (!sip_list_eol(&media->m_fmt_list, pos))
	{
		char *str = (char *) sip_list_get(&media->m_fmt_list, pos);
	...
	//tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
/********************************************************************/
	while (!sip_list_eol(&media->m_payloads, pos)) {
		char *str = (char *) sip_list_get(&media->m_payloads, pos);
	...
	if (media->i_info != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "i=");
		tmp = __sip_sdp_append_string(string, size, tmp, media->i_info);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}

	pos = 0;
	while (!sip_list_eol(&media->c_connections, pos)) {
		sdp_connection_t *conn =
			(sdp_connection_t *) sip_list_get(&media->c_connections, pos);
		char *next_tmp2;
		int i;
	...
	pos = 0;
	while (!sip_list_eol(&media->b_bandwidths, pos)) {
		sdp_bandwidth_t *band =
			(sdp_bandwidth_t *) sip_list_get(&media->b_bandwidths, pos);
		char *next_tmp2;
		int i;
	...
	pos = 0;
	while (!sip_list_eol(&media->a_attributes, pos)) {
		sdp_attribute_t *attr =
			(sdp_attribute_t *) sip_list_get(&media->a_attributes, pos);
		char *next_tmp2;
		int i;
...
int sdp_message_to_str(sdp_message_t * sdp, char **dest)
{
	...
	*dest = NULL;
	...
	tmp = (char *) sip_malloc(size);
	if (tmp == NULL)
		return sip_NOMEM;
	string = tmp;

	tmp = __sip_sdp_append_string(string, size, tmp, "v=");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->v_version);
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	tmp = __sip_sdp_append_string(string, size, tmp, "o=");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_username);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_sess_id);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_sess_version);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_nettype);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_addrtype);
	tmp = __sip_sdp_append_string(string, size, tmp, " ");
	tmp = __sip_sdp_append_string(string, size, tmp, sdp->o_addr);
	tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	if (sdp->s_name != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "s=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->s_name);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}
	if (sdp->i_info != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "i=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->i_info);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}
	if (sdp->u_uri != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "u=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->u_uri);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}
	pos = 0;
	while (!sip_list_eol(&sdp->e_emails, pos)) {
		char *email = (char *) sip_list_get(&sdp->e_emails, pos);

		tmp = __sip_sdp_append_string(string, size, tmp, "e=");
		tmp = __sip_sdp_append_string(string, size, tmp, email);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
		pos++;
	}
	pos = 0;
	while (!sip_list_eol(&sdp->p_phones, pos)) {
		char *phone = (char *) sip_list_get(&sdp->p_phones, pos);

		tmp = __sip_sdp_append_string(string, size, tmp, "p=");
		tmp = __sip_sdp_append_string(string, size, tmp, phone);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
		pos++;
	}
	...
	pos = 0;
	while (!sip_list_eol(&sdp->b_bandwidths, pos)) {
		sdp_bandwidth_t *header =
			(sdp_bandwidth_t *) sip_list_get(&sdp->b_bandwidths, pos);
		char *next_tmp;
		int i;
	...
	pos = 0;
	while (!sip_list_eol(&sdp->t_descrs, pos)) {
		sdp_time_descr_t *header =
			(sdp_time_descr_t *) sip_list_get(&sdp->t_descrs, pos);
		char *next_tmp;
		int i;
	...
	if (sdp->z_adjustments != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "z=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->z_adjustments);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}
	...
	pos = 0;
	while (!sip_list_eol(&sdp->a_attributes, pos)) {
		sdp_attribute_t *header =
			(sdp_attribute_t *) sip_list_get(&sdp->a_attributes, pos);
		char *next_tmp;
		int i;
	...
	pos = 0;
	while (!sip_list_eol(&sdp->m_medias, pos)) {
		sdp_media_t *header = (sdp_media_t *) sip_list_get(&sdp->m_medias, pos);
		char *next_tmp;
		int i;
	...
	if (sdp->f_value != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "f=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->f_value);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}

    if (sdp->y_value != NULL) {
		tmp = __sip_sdp_append_string(string, size, tmp, "y=");
		tmp = __sip_sdp_append_string(string, size, tmp, sdp->y_value);
		tmp = __sip_sdp_append_string(string, size, tmp, CRLF);
	}
	*dest = string;
	return sip_SUCCESS;
...
void sdp_message_free(sdp_message_t * sdp)
{
	if (sdp == NULL)
		return;
	sip_free(sdp->v_version);
	sip_free(sdp->o_username);
	sip_free(sdp->o_sess_id);
	sip_free(sdp->o_sess_version);
	sip_free(sdp->o_nettype);
	sip_free(sdp->o_addrtype);
	sip_free(sdp->o_addr);
	sip_free(sdp->s_name);
	sip_free(sdp->i_info);
	sip_free(sdp->u_uri);
	sip_free(sdp->f_value);
    sip_free(sdp->y_value);
	...
	sip_free(sdp->z_adjustments);
	sdp_key_free(sdp->k_key);

	sip_list_special_free(&sdp->a_attributes,
						   (void (*)(void *)) &sdp_attribute_free);

	sip_list_special_free(&sdp->m_medias, (void (*)(void *)) &sdp_media_free);
